...
def run(
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    if save_txt:  # Write to file
                        ...
                        class_id = int(det[5])
                        class_label = class_descriptions[class_id]
                        conf = det[4]
                        LOGGER.info(f"Detected {class_label} with confidence {conf:.2f}.")
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        for i, det in enumerate(pred): 
                            for i, det in enumerate(pred):
                                ...
                                description = descriptions[class_id] if class_id < len(descriptions) else "Unknown"
                                label = f"{names[class_id]} - {description}  "
    ...
    LOGGER.info(f"Detection completed: {seen} images processed with {len(det)} detections.")
